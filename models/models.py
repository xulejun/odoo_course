...
class Teacher(models.Model):
    ...
    className = fields.Char(string=u"你的课程", default=lambda self: self.env['course.session'].search(
        [('course_id', '=', self.env.user.name)]).name)
    name = fields.Char(string=u'姓名', default=lambda self: self.env.user.name)
    address = fields.Char(string=u'家庭住址')
    mobile_phone = fields.Char(string=u'手机号码')
    work_email = fields.Char(string=u'工作邮箱')
...
class leave(models.Model):
    ...
    sumdays = fields.Integer(string=u"课程总节数", default=lambda self: self.env['course.session'].search(
        [('course_id', '=', self.env.user.classname)]).num)
...
class Session(models.Model):
    ...
    course_id = fields.Char(string="任课老师", related='test_id.name', store=True)
    test_id = fields.Many2one('course.teacher', string='任课老师', required=True)
    attendee_ids = fields.One2many('course.choice', 'leader_id', string="上课学生")
    num = fields.Integer(string="课程节数", required=True, default=10)
